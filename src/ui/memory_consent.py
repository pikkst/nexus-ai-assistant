...
class MemoryConsentWindow(ctk.CTk):
    ...
    def __init__(
        ...
        self.title("Nexus — Memory Settings")
    ...
    def _build_ui(self) -> None:
        ...
        ctk.CTkLabel(
            container, text="Memory Management", font=("Arial", 20, "bold")
        ).pack(anchor="w", pady=(0, 8))

        filters_frame = ctk.CTkFrame(container)
        filters_frame.pack(fill="x", pady=(0, 8))

        ctk.CTkLabel(filters_frame, text="Search:").grid(row=0, column=0, padx=4, pady=4, sticky="w")
        ...
        ctk.CTkLabel(filters_frame, text="Type:").grid(row=0, column=2, padx=4, pady=4, sticky="w")
        ...
        ctk.CTkLabel(filters_frame, text="Scope:").grid(row=0, column=4, padx=4, pady=4, sticky="w")
        ...
        ctk.CTkLabel(filters_frame, text="Source:").grid(row=1, column=0, padx=4, pady=4, sticky="w")
        ...
        ctk.CTkLabel(filters_frame, text="Sensitivity:").grid(row=1, column=2, padx=4, pady=4, sticky="w")
        ...
        ctk.CTkButton(btn_frame, text="Delete", command=self._delete_selected).pack(
            side="left", padx=4
        )
        ctk.CTkButton(btn_frame, text="Edit", command=self._edit_selected).pack(
            side="left", padx=4
        )
        ctk.CTkButton(btn_frame, text="Save Changes", command=self._save_edit).pack(
            side="left", padx=4
        )
        ctk.CTkButton(btn_frame, text="Pin / Unpin", command=self._toggle_pin).pack(
            side="left", padx=4
        )
        ctk.CTkButton(btn_frame, text="Memory Summary", command=self._show_summary).pack(
            side="left", padx=4
        )
        ...
        ctk.CTkLabel(export_frame, text="Export:").pack(side="left", padx=4)
        ...
        ctk.CTkButton(export_frame, text="Export by Type", command=self._export_type).pack(
            side="left", padx=4
        )

        ctk.CTkLabel(export_frame, text="Clear:").pack(side="left", padx=4)
        ...
        ctk.CTkButton(export_frame, text="Clear by Type", command=self._clear_type).pack(
            side="left", padx=4
        )
        ...
        ctk.CTkLabel(consent_frame, text="Sensitive Data Policy:").pack(side="left", padx=4)
        ctk.CTkOptionMenu(
            consent_frame, variable=self._consent_action, values=CONSENT_ACTIONS,
            command=self._on_consent_change,
        ).pack(side="left", padx=4)

        ctk.CTkButton(container, text="Close", command=self.destroy).pack(
            fill="x", pady=(8, 0)
        )
    ...
    def _selected_entry(self) -> MemoryEntry | None:
        index_str = self._memory_listbox.index("insert")
        try:
            line = int(index_str.split(".")[0])
        except (ValueError, IndexError):
            return None
        if line < 1 or line > len(self._current_entries):
            return None
        return self._current_entries[line - 1]
    ...
    def _edit_selected(self) -> None:
        ...
        if entry is not None:
            ...
            self._memory_listbox.delete("1.0", "end")
            self._memory_listbox.insert("end", f"Edit entry {entry.id}:\n")
