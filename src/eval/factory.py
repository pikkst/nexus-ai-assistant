...
def create_eval_layer(path: Path | str = Path("~/.nexus/eval")) -> tuple[EvalStore, StepVerifier, FeedbackCollector, MetricsTracker]:
    store = EvalStore(path)
    return store, StepVerifier(), FeedbackCollector(), MetricsTracker()
