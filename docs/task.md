...
- [ ] **AC-1:** STT service that accepts audio chunks or live microphone stream
...
- Module integrates cleanly with the audio capture pipeline
...
- [ ] **AC-1:** LLM client configurable for local model backend such as Ollama
...
Implement persistent memory for the assistant using a lightweight vector store or similar retrieval layer so the system can remember prior interactions and retrieve relevant context.
...
- [ ] **AC-1:** Main window or web UI shell with face area and conversation view
- [ ] **AC-2:** Controls for start/stop, mute, and settings access
- [ ] **AC-3:** Live status indicators for listening, speaking, and thinking
- [ ] **AC-4:** Integration with existing face demo and settings panel
...
- [ ] **AC-4:** Configuration-driven wiring for enabled/disabled features
